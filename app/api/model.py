...
from app.core.model.service import create_model, delete_model, \
    find_default_model, \
    set_or_unset_default_model, test_run_model, update_model, \
    update_model_enable
...
def update_enable(session: SessionDep, current_user: CurrentUser,
        id: int) -> Any:
    res = update_model_enable(session, id, current_user)
    return ApiResult.create(res)
...
def set_or_unset_default(session: SessionDep, params: SetupDefaultModel,
